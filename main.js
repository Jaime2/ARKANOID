...
//${METADATA_BEGIN}
var META_DATA="[mojo_font.png];type=image/png;width=864;height=13;\n[ball.png];type=image/png;width=26;height=26;\n[bat.png];type=image/png;width=100;height=28;\n[bricks.png];type=image/png;width=75;height=150;\n[font.png];type=image/png;width=172;height=22;\n";
//${METADATA_END}
...
function c_MyApp(){
	...
	this.m_bat_img=null;
	this.m_ball=null;
	this.m_bricks=new_object_array(20);
	this.m_hit_sfx=null;
	this.m_smash_sfx=null;
	this.m_font_img=null;
	this.m_batx=250.0;
	this.m_prev_batx=250.0;
	this.m_baty=450.0;
	this.m_score=0;
	this.m_cleared=0;
}
...
c_MyApp.prototype.p_OnCreate=function(){
	bb_autofit_SetVirtualDisplay(500,500,1.0);
	bb_app_SetUpdateRate(60);
	print("Lets dev!");
	this.m_bat_img=bb_graphics_LoadImage("bat.png",1,1);
	this.m_ball=c_Ball.m_new.call(new c_Ball,250.0,250.0);
	for(var t_b=0;t_b<4;t_b=t_b+1){
		for(var t_r=0;t_r<5;t_r=t_r+1){
			var t_brk=c_Brick.m_new.call(new c_Brick,100.0+(t_r)*75.0,100.0+(t_b)*25.0,((bb_random_Rnd3(6.0))|0));
			this.m_bricks[t_r+t_b*5]=t_brk;
		}
	}
	this.m_hit_sfx=bb_audio_LoadSound("hit.wav");
	this.m_smash_sfx=bb_audio_LoadSound("smash.wav");
	this.m_font_img=bb_graphics_LoadImage("font.png",10,c_Image.m_DefaultFlags);
	bb_graphics_SetFont(this.m_font_img,48);
	return 0;
}
c_MyApp.prototype.p_ResetBoard=function(){
	var t_=this.m_bricks;
	var t_2=0;
	while(t_2<t_.length){
		var t_brk=t_[t_2];
		t_2=t_2+1;
		t_brk.m_hidden=false;
	}
}
c_MyApp.prototype.p_OnUpdate=function(){
	this.m_prev_batx=this.m_batx;
	if(bb_autofit_VMouseX(true)>0.0){
		this.m_batx=bb_autofit_VMouseX(true);
	}
	this.m_ball.p_Update();
	if(bb_math_Abs2(this.m_ball.m_posx-this.m_batx)<50.0 && bb_math_Abs2(this.m_ball.m_posy-this.m_baty)<28.0 && this.m_ball.m_vely>0.0){
		this.m_ball.m_vely*=-1.0;
		bb_audio_PlaySound(this.m_hit_sfx,0,0);
		this.m_ball.m_velx+=0.25*(this.m_batx-this.m_prev_batx);
	}
	var t_=this.m_bricks;
	var t_2=0;
	while(t_2<t_.length){
		...
		t_2=t_2+1;
		if(!t_brick.m_hidden){
			var t_yd=bb_math_Abs2(this.m_ball.m_posy-t_brick.m_posy);
			var t_xd=bb_math_Abs2(this.m_ball.m_posx-t_brick.m_posx);
			if(t_xd<=50.5 && t_yd<12.5){
				this.m_ball.m_velx*=-1.0;
				t_brick.m_hidden=true;
				this.m_score+=100;
				bb_audio_PlaySound(this.m_smash_sfx,0,0);
				this.m_cleared+=1;
			}else{
				if(t_yd<=25.5 && t_xd<37.5){
					this.m_ball.m_vely*=-1.0;
					t_brick.m_hidden=true;
					this.m_score+=100;
					this.m_cleared+=1;
	...
	if(this.m_cleared>=20){
		this.p_ResetBoard();
		this.m_score+=1000;
		this.m_cleared=0;
	}
	if(this.m_ball.m_posy>460.0){
		this.p_ResetBoard();
		this.m_cleared=0;
		this.m_score=0;
		this.m_ball.m_posx=250.0;
		this.m_ball.m_posy=250.0;
		this.m_ball.m_velx=0.0;
		this.m_ball.m_vely=4.0;
	}
	return 0;
}
c_MyApp.prototype.p_OnRender=function(){
	...
	bb_graphics_DrawImage(this.m_bat_img,this.m_batx,this.m_baty,0);
	this.m_ball.p_Draw();
	var t_=this.m_bricks;
	var t_2=0;
	while(t_2<t_.length){
		var t_brk=t_[t_2];
		t_2=t_2+1;
		if(((t_brk)!=null) && !t_brk.m_hidden){
			t_brk.p_Draw();
		}
	}
	bb_graphics_DrawText(String(this.m_score),250.0,40.0,0.5,0.5);
	return 0;
}
...
c_VirtualDisplay.prototype.p_UpdateVirtualDisplay=function(t_zoomborders,t_keepborders){
	...
	if(t_zoomborders){
		...
		t_sh=bb_math_Min2(t_sh,this.m_fdh);
		...
		t_sh=bb_math_Min2(this.m_fdh-this.m_heightborder*2.0,this.m_fdh);
		bb_graphics_SetScissor(t_sx,t_sy,t_sw,t_sh);
	}
...
c_Ball.m_ball_img=null;
c_Ball.m_new=function(t_x,t_y){
	this.m_posx=t_x;
	this.m_posy=t_y;
	if(!((c_Ball.m_ball_img)!=null)){
		c_Ball.m_ball_img=bb_graphics_LoadImage("ball.png",1,1);
	}
	return this;
}
...
c_Ball.prototype.p_Update=function(){
	...
	if(this.m_posx>500.0 && this.m_velx>0.0){
		this.m_velx*=-1.0;
	}
	if(this.m_posx<0.0 && this.m_velx<0.0){
		this.m_velx*=-1.0;
	}
	if(this.m_posy>500.0 && this.m_vely>0.0){
		this.m_vely*=-1.0;
	}
	if(this.m_posy<0.0 && this.m_vely<0.0){
		this.m_vely*=-1.0;
	}
}
c_Ball.prototype.p_Draw=function(){
	bb_graphics_DrawImage(c_Ball.m_ball_img,this.m_posx,this.m_posy,0);
}
function c_Brick(){
	Object.call(this);
	this.m_posx=.0;
...
c_Brick.m_bricks_img=null;
c_Brick.m_new=function(t_x,t_y,t_c){
	this.m_posx=t_x;
	this.m_posy=t_y;
	this.m_colour=t_c;
	if(!((c_Brick.m_bricks_img)!=null)){
		c_Brick.m_bricks_img=bb_graphics_LoadImage("bricks.png",1,c_Image.m_DefaultFlags);
	}
	return this;
}
...
c_Brick.prototype.p_Draw=function(){
	bb_graphics_DrawImageRect(c_Brick.m_bricks_img,this.m_posx-37.5,this.m_posy-12.5,0,(((this.m_colour)*25.0)|0),75,25,0);
}
